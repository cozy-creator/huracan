use anyhow::Result;
use pulsar::{
	compression::*, consumer::InitialPosition, producer, Authentication, Consumer, ConsumerOptions, DeserializeMessage,
	Producer, Pulsar, SubType, TokioExecutor,
};
...
async fn create_pulsar(uri: &str, token: &Option<String>) -> Result<Pulsar<TokioExecutor>> {
	...
	if let Some(token) = &token {
		let auth = Authentication { name: "token".to_string(), data: token.clone().into_bytes() };
...
pub async fn create_pulsar_producer(options: &PulsarProducerOptions) -> Result<Producer<TokioExecutor>> {
	...
	producer.check_connection().await.context("cannot check apache pulsar connection")?;
